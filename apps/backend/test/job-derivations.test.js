...
test("strict drilldown sanitization keeps only classification research fields",()=>{const out=sanitizeDrilldownRecords([{JobId:77,JobTypeId:1,JobTypeName:"Service",BusinessUnitId:3,Status:"Completed",Revenue:"$10",CustomerName:"Jane",Phone:"555",Email:"x@y.com",Notes:"private",XUnknown:"no"}]);assert.deepEqual(out.records,[{recordKey:77,jobTypeId:1,jobTypeName:"Service",businessUnitId:3,status:"Completed",revenue:"$10"}]);assert.ok(out.removedFields.includes("CustomerName"));assert.ok(out.removedFields.includes("XUnknown"));assert.doesNotMatch(JSON.stringify(out),/Jane|555|x@y|private/);});
