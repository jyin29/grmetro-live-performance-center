...
const DEFAULTS = Object.freeze({
  ...
  SERVICETITAN_RELOAD_KEY: "2",
  SERVICETITAN_DRILLDOWN_KPI_TYPE: "2",
  // Backwards-compatible deployment default. Override this in .env before packaging
  // for another company; once every installation has local config, this fallback can
  // be removed from source control without changing request-building code.
  SERVICETITAN_BUSINESS_UNIT_IDS: "127825768,1103,1107,1105,1111,1113,1109,1123,3586,3588,3589,3587,1119,1115,1121,1117,1095,1099,1101,1097",
  ...
  REMOTE_RATE_LIMIT_WINDOW_SECONDS: "60",
  REMOTE_RATE_LIMIT_MAX_REQUESTS: "30",
  LOG_LEVEL: "info"
});
...
function integerList(value, name) {
  ...
  if (!items.length) throw new Error(`${name} must contain at least one numeric ID.`);
  const parsed = items.map((item) => integer(item, name));
...
function validateHost(value) {
  ...
  if (!net.isIP(unwrapped) && !hostname.test(value)) throw new Error("HOST must be a hostname or IP address without a protocol or port.");
  return value;
}
...
function loadConfig(environment = process.env, options = {}) {
  ...
  return Object.freeze({
    ...
    host: validateHost(env.HOST),
    port: integer(env.PORT, "PORT", { max: 65535 }),
    ...
    serviceTitanRequestTimeoutMilliseconds: integer(env.SERVICETITAN_REQUEST_TIMEOUT_MILLISECONDS, "SERVICETITAN_REQUEST_TIMEOUT_MILLISECONDS", { min: 1000, max: 120000 }),
    serviceTitanCsrfTimeoutMilliseconds: integer(env.SERVICETITAN_CSRF_TIMEOUT_MILLISECONDS, "SERVICETITAN_CSRF_TIMEOUT_MILLISECONDS", { min: 100, max: 120000 }),
    serviceTitanReloadKey: String(env.SERVICETITAN_RELOAD_KEY),
    serviceTitanDrilldownKpiType: String(env.SERVICETITAN_DRILLDOWN_KPI_TYPE),
    businessUnitIds: integerList(env.SERVICETITAN_BUSINESS_UNIT_IDS, "SERVICETITAN_BUSINESS_UNIT_IDS"),
    ...
    staleWarningSeconds: warning,
    staleCriticalSeconds: critical,
    ...
    remoteRateLimit: Object.freeze({
      windowSeconds: integer(env.REMOTE_RATE_LIMIT_WINDOW_SECONDS, "REMOTE_RATE_LIMIT_WINDOW_SECONDS", { max: 3600 }),
      maxRequests: integer(env.REMOTE_RATE_LIMIT_MAX_REQUESTS, "REMOTE_RATE_LIMIT_MAX_REQUESTS", { max: 10000 })
    }),
    logLevel: env.LOG_LEVEL,
    mockMode: mock.mockMode,
    developmentRoutesEnabled: mock.developmentRoutesEnabled
  });
}
