...
function leadGenerationKpi(item) {
  const lead = item?.leadGeneration?.all;
  const value = Number(lead?.convRate);
...
class ServiceTitanRefreshProvider {
  ...
  async refreshTechnician(technician, now, dateRange, scorecardById = new Map()) {
    ...
    try {
      ...
      const leadValue = leadGenerationKpi(scorecardById.get(technician.id));
      if (leadValue !== null) kpis.leadConversionRate = { ...kpis.leadConversionRate, value: leadValue, hasData: true, dataQuality: "confirmed" };
  ...
  async refresh({ now = new Date().toISOString(), date, period = "today", dateRange, previousPayload } = {}) {
    ...
    try {
      const response = await this.executor.post(ENDPOINTS.allScorecard, buildAllScorecardRequest(this.config, new Date(now), dateRange)); const scorecard = validateJsonResponse(response, { endpointName: ENDPOINTS.allScorecard.name, expectedShape: "object" });
      scorecardById = new Map([...(scorecard.originalItems || []), ...(scorecard.items || [])].map((item) => [scorecardId(item), item]).filter(([id]) => Number.isFinite(id)));
    ...
    const results = await mapLimited(this.technicians, this.concurrency, (technician) => this.refreshTechnician(technician, now, dateRange, scorecardById));
