...
function findServiceTitanPage(browser,{preferredPage=null}={}){if(isSupportedDashboardPage(preferredPage))return preferredPage;const modular=[],scorecards=[];let sawUnauthenticated=false;for(const context of browser?.contexts?.()||[])for(const page of context.pages?.()||[]){const raw=rawPageUrl(page),url=pageUrl(page);if(!url||!isServiceTitan(url))continue;if(appearsUnauthenticated(url)){sawUnauthenticated=true;continue;}const kind=dashboardKind(url,raw);if(kind==="modular-dashboard")modular.push(page);else if(kind==="technician-scorecard")scorecards.push(page);}const selected=modular[0]||scorecards[0];if(selected&&!selected.isClosed?.())return selected;const error=new BrowserManagerError(sawUnauthenticated?"SERVICE_TITAN_AUTH_REQUIRED":"SERVICE_TITAN_PAGE_NOT_FOUND");error.visiblePages=inspectBrowserPages(browser);throw error;}
