...
const presentationSlides = require("../../../shared/slides");
...
const { createDisplayPresence } = require("./routes/presentationRoutes");
...
function start() {
  ...
  const provider = createRefreshProvider({
    config,
    browserManager,
    executor: serviceTitanClient?.executor,
    logger,
    goalsProvider: () => goalStore.getGoals(scheduler?.getPeriod?.() || "today"),
  });
  ...
  const presentationManager = createPresentationManager({
    displays: PRESENTATION_DISPLAYS,
    slideCount: PRESENTATION_SLIDE_COUNT,
    rotationMilliseconds: PRESENTATION_ROTATION_MILLISECONDS,
    eventEngine,
    slideIds: presentationSlides.map(({ id }) => id),
    getEligibleSlideIndices: () => presentationSlides.flatMap((slide, index) => slide.id === "spreadsheet" && !spreadsheetSlideStore.isAvailable() ? [] : [index]),
    getSlideAvailabilityRevision: () => spreadsheetSlideStore.getPublicState().revision,
    subscribeSlideEligibility: (listener) => spreadsheetSlideStore.subscribe(listener),
  });
  const presentationCommandBus = createPresentationCommandBus({ handleCommand: presentationManager.handleCommand });
  const displayPresence = createDisplayPresence();
  let presentationWebSocket;
  scheduler = provider ? new RefreshScheduler({
    provider,
    cache,
    logger,
    intervalMilliseconds: config.refreshIntervalSeconds * 1000,
    timeZone: config.timeZone,
    onSuccessfulPayload: (payload) => {
  ...
  const browserStatusProvider = browserManager ? () => browserManager.getStatus() : mockBrowserStatus;
  const serviceTitanStatusProvider = serviceTitanClient ? () => serviceTitanClient.getStatus() : () => ({ status: "bypassed" });
  const connectionStatusProvider = () => {
    const ws = presentationWebSocket?.getConnectionSummary() || { total: 0, displays: 0, remotes: 0, byDisplay: {} };
    const byDisplay = { ...ws.byDisplay };
    for (const id of displayPresence.getOnlineDisplayIds()) {
      const current = byDisplay[id] || { displays: 0, remotes: 0, total: 0 };
      byDisplay[id] = { ...current, displays: Math.max(1, current.displays || 0), total: Math.max(1, current.total || 0) };
    }
    const displays = Object.values(byDisplay).filter((value) => (value.displays || 0) > 0).length;
    return { ...ws, displays, byDisplay };
  };
  const app = createApp({
    config,
    logger,
    ...
    serviceTitanStatusProvider,
    serviceTitanClient,
    goalStore,
    displaySettingsStore,
    spreadsheetSlideStore,
    presentationRuntime: { presentationManager, commandBus: presentationCommandBus, displayPresence },
    adminRuntime: { presentationManager, displayPresence, eventEngine, startedAt, connectionStatusProvider, scheduler, browserStatusProvider, serviceTitanStatusProvider },
  });
  ...
  presentationWebSocket = createPresentationWebSocket({ server, manager: presentationManager, commandBus: presentationCommandBus, logger, displayPresence });
  installGracefulShutdown({ server, logger, scheduler: { stop() {
    scheduler?.stop();
    expirationMonitor.stop();
    presentationWebSocket.close();
    presentationManager.destroy();
    eventEngine.destroy();
    serviceTitanClient?.stop();
    browserManager?.stop();
  } } });
