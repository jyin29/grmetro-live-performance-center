...
const MEMORY_CHECK_MS = 30 * 1000;
const MEMORY_RATIO_WARNING = 0.80;
const MEMORY_WARNINGS_BEFORE_RELOAD = 3;
const MAINTENANCE_RELOAD_MS = 6 * 60 * 60 * 1000;
...
export function installKioskRecovery({enabled=true}={}){
  ...
  const check=async()=>{
    ...
    try{
      ...
      backendHealthy=true;
      if(backendFailures){log("backend-recovered",{failures:backendFailures});backendFailures=0;}
      const uptimeMs=Date.now()-startedAt;
      if(shouldControlledReload({backendHealthy,runtimeErrors,memoryWarnings,uptimeMs})){
        const reason=memoryWarnings>=MEMORY_WARNINGS_BEFORE_RELOAD?"sustained-memory-pressure-with-healthy-backend":runtimeErrors>=RUNTIME_ERRORS_BEFORE_RELOAD?"runtime-errors-with-healthy-backend":"scheduled-long-runtime-refresh";
        if(attemptControlledReload(runtimeErrors,reason))return;
        runtimeErrors=0;memoryWarnings=0;
      }
    }catch(error){
      if(error.name==="AbortError")return;
      backendHealthy=false;backendFailures+=1;
      log("backend-health-missed",{failures:backendFailures,action:"preserve-last-known-good"});
    }
  };
  const onError=event=>{runtimeErrors+=1;log("browser-runtime-error",{message:String(event?.message||event?.reason||"unknown"),runtimeErrors});};
  const memoryCheck=()=>{
    const memory=performance?.memory;
    if(!memory||!Number.isFinite(memory.usedJSHeapSize)||!Number.isFinite(memory.jsHeapSizeLimit)||memory.jsHeapSizeLimit<=0)return;
    const ratio=memory.usedJSHeapSize/memory.jsHeapSizeLimit;
    if(ratio>=MEMORY_RATIO_WARNING){memoryWarnings+=1;log("memory-pressure",{percent:Math.round(ratio*100),warnings:memoryWarnings,action:backendHealthy?"eligible-for-controlled-recovery":"preserve-last-known-good"});}
    else if(memoryWarnings){memoryWarnings=0;log("memory-pressure-recovered",{percent:Math.round(ratio*100)});}
  };
  const timer=window.setInterval(check,CHECK_MS),memoryTimer=window.setInterval(memoryCheck,MEMORY_CHECK_MS);
  check();memoryCheck();window.addEventListener("error",onError);window.addEventListener("unhandledrejection",onError);
  return()=>{stopped=true;window.clearInterval(timer);window.clearInterval(memoryTimer);request?.abort();window.removeEventListener("error",onError);window.removeEventListener("unhandledrejection",onError);};
}
