...
export function usePresentationController(requestedDisplayId=DEFAULT_DISPLAY_ID,clientType="display"){
  ...
  // HTTP state polling is deliberately kept alongside WebSocket. It makes the TV/remote
  // recover and remain controllable on LANs/browsers where a WebSocket is interrupted.
  ...
  // A display proves that the actual screen is open. Remote clients never send this heartbeat.
  useEffect(()=>{if(clientType!=="display")return undefined;let active=true;const beat=()=>{if(!active)return;fetch(`/api/v1/presentation/${encodeURIComponent(displayId)}/heartbeat`,{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"}).catch(()=>{});};beat();const timer=window.setInterval(beat,HEARTBEAT_MS);return()=>{active=false;window.clearInterval(timer);};},[clientType,displayId]);

  useEffect(()=>{rememberSlideIndex(displayId,state.activeSlideIndex);},[displayId,state.activeSlideIndex]);
  const send=useCallback(async(type,payload)=>{const command=createPresentationCommand(type,displayId,payload);try{transport?.send(command);return;}catch{/* HTTP fallback below */}const response=await fetch(`/api/v1/presentation/${encodeURIComponent(displayId)}/command`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(command)});if(!response.ok)throw new Error("Presentation command failed.");const result=await response.json();acceptState(result.state);},[acceptState,displayId,transport]);
