...
export function formatMetric(metric) {
  ...
  if (metric.format === "currency") return CURRENCY_FORMATTER.format(metric.value);
  if (metric.format === "percentage") return `${PERCENT_FORMATTER.format(metric.value)}%`;
...
export function managementInsights(data, presentationState = {}, now = Date.now(), limit = 2) {
  const preparedInsights = Array.isArray(data?.managementInsights) ? data.managementInsights : data?.managementInsights?.insights;
  ...
  if (presentationState.hasError) add({ id: "feed-interrupted", priority: "critical", eyebrow: "Feed health", title: "Live updates interrupted", detail: "Showing the last successful dashboard update." });
  else if (freshnessState === "critical") add({ id: "feed-critical", priority: "critical", eyebrow: "Feed health", title: "Live data needs attention", detail: data?.refreshedAt ? "The last successful update is more than 10 minutes old." : "No successful dashboard update is available." });
  else if (freshnessState === "stale") add({ id: "feed-stale", priority: "warning", eyebrow: "Feed health", title: "Dashboard data is delayed", detail: "The last successful update is more than 3 minutes old." });
  const ranked = rankedTechnicians(data?.technicians);
  const falling = ranked.filter(({ overall }) => overall?.qualifies && overall.rankChange < 0).sort((left, right) => left.overall.rankChange - right.overall.rankChange || left.overall.rank - right.overall.rank)[0];
  ...
  for (const technician of data?.technicians || []) for (const [id, metric] of Object.entries(technician.kpis || {})) if (metric?.dataQuality === "fallback" || metric?.dataQuality === "unavailable") qualityIds.add(id);
  if (qualityIds.size) {
  ...
  const climbing = ranked.filter(({ overall }) => overall?.qualifies && overall?.rankChange > 0).sort((left, right) => right.overall.rankChange - left.overall.rankChange || left.overall.rank - right.overall.rank)[0];
...
export function operationsHealthPresentation(data, presentationState = {}, now = Date.now()) {
  ...
  return {
    overall: { label: isHealthy ? "Dashboard Healthy" : hasError ? "Updates Interrupted" : "Data Needs Attention", detail: isHealthy ? "Live data is current and the presentation is running." : hasError ? "Showing the last successful dashboard update." : data?.refreshedAt ? "The latest dashboard data is older than expected." : "Waiting for the first successful update.", tone: isHealthy ? "healthy" : "warning" },
    cards: [
      { id: "refresh", label: "Last Successful Refresh", value: hasRefreshTime ? refreshLabel(data.refreshedAt, now).replace(/^Updated /, "") : "Unavailable", detail: hasRefreshTime ? CLOCK_FORMATTER.format(refreshTime) : "No successful refresh yet", tone: freshnessState === "live" ? "healthy" : "warning" },
      { id: "refresh-state", label: "Refresh State", value: presentationState.refreshing ? "Refreshing" : hasError ? "Interrupted" : "Ready", detail: presentationState.refreshing ? "Updating cached dashboard data" : hasError ? "Last update remains visible" : "Waiting for the next refresh", tone: hasError ? "warning" : "healthy" },
      { id: "cache", label: "Cache Status", value: data ? "Available" : "Unavailable", detail: data ? "Dashboard data is ready" : "No dashboard payload available", tone: data ? "healthy" : "warning" },
      { id: "technicians", label: "Technicians", value: Array.isArray(data?.technicians) ? String(data.technicians.length) : "Unavailable", detail: Array.isArray(data?.technicians) ? "Included in this dashboard" : "Technician count unavailable", tone: Array.isArray(data?.technicians) ? "healthy" : "neutral" },
      { id: "slide", label: "Current Slide", value: "Operations Health", detail: "Slide 5 of 5", tone: "neutral" },
      { id: "rotation", label: "Rotation Status", value: presentationState.rotationPaused ? "Paused" : "Running", detail: presentationState.rotationPaused ? "Resumes when updates recover" : "30-second presentation rotation", tone: presentationState.rotationPaused ? "warning" : "healthy" }
    ]
  };
}
...
export function rankedTechnicians(technicians = []) {
  return [...technicians].sort((left, right) => {
    ...
    return (leftRank ?? Number.POSITIVE_INFINITY) - (rightRank ?? Number.POSITIVE_INFINITY) || String(left.name || "").localeCompare(String(right.name || "")) || String(left.id).localeCompare(String(right.id));
  });
}

export function displayRank(technician) {
  if (technician?.overall?.qualifies && technician.overall.rank) return { rank: technician.overall.rank, rankLabel: technician.overall.rankLabel || `#${technician.overall.rank}`, label: "Overall Rank", source: "overall", overallStatus: "Qualified" };
  if (technician?.kpis?.revenue?.rank) return { rank: technician.kpis.revenue.rank, rankLabel: technician.kpis.revenue.rankLabel || `#${technician.kpis.revenue.rank}`, label: "Revenue Rank", source: "revenue", overallStatus: "Not qualified" };
...
export function summaryMetrics(data) {
  const definitions = [["revenue", "Revenue"], ["billableServiceCalls", "Billable Calls"], ["closingRate", "Closing"], ["installAverageTicket", "Install Avg Ticket"]];
  return definitions.map(([id, label]) => {
    const ranked = data?.technicians?.filter((technician) => technician.kpis?.[id]?.hasData).sort((a, b) => (a.kpis[id].rank ?? Infinity) - (b.kpis[id].rank ?? Infinity));
...
export function recognitionPresentation(technicians = []) {
  ...
  const metricLeader = (id) => technicians.find((technician) => technician.kpis?.[id]?.hasData && technician.kpis[id].rank === 1);
  const biggestImprovement = [...technicians].filter((technician) => technician.overall?.qualifies && technician.overall?.rankChange > 0).sort((left, right) => right.overall.rankChange - left.overall.rankChange || left.overall.rank - right.overall.rank)[0];
  const recognitions = [
