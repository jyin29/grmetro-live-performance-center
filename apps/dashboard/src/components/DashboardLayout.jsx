...
export function DashboardLayout({ data, displayId, displaySettings, error, refreshing, retry, lastSuccessfulRefresh }) {
  ...
  const watchdog = useMemo(() => createDisplayWatchdog({
    intervalMs: RUNTIME_SETTINGS.watchdogIntervalMs,
    presentationStaleMs: RUNTIME_SETTINGS.presentationStaleMs,
    dashboardStaleMs: RUNTIME_SETTINGS.dashboardStaleMs,
    onRecover: ({ disconnected, presentationStale, dashboardStale }) => {
      if (disconnected || presentationStale) recoveryRef.current.presentation.reconnect();
      if (dashboardStale) recoveryRef.current.retry({ background: true });
    },
  }), []);

  useEffect(() => { watchdog.start(); return () => watchdog.stop(); }, [watchdog]);
  useEffect(() => { watchdog.update({ connected: presentation.connectionState === "connected", lastSynchronization: presentation.lastSynchronization, lastDashboardRefresh: lastSuccessfulRefresh }); },
    [lastSuccessfulRefresh, presentation.connectionState, presentation.lastSynchronization, watchdog]);
  useEffect(() => attachWakeRecovery({ recover: () => watchdog.wake("browser-wake") }), [watchdog]);
  useEffect(() => {
    let cursorTimer;
    const showCursor = () => { document.body.classList.remove("cursor-hidden"); window.clearTimeout(cursorTimer); cursorTimer = window.setTimeout(() => document.body.classList.add("cursor-hidden"), RUNTIME_SETTINGS.cursorIdleMs); };
    const preventDrag = (event) => event.preventDefault();
    const onKeyDown = (event) => { if (isFullscreenShortcut(event)) { event.preventDefault(); toggleDisplayFullscreen(); } if (event.shiftKey && event.key.toLowerCase() === "d") setDiagnosticsVisible((visible) => !visible); };
    window.addEventListener("pointermove", showCursor, { passive: true }); window.addEventListener("keydown", onKeyDown); document.addEventListener("dragstart", preventDrag);
    showCursor(); if (RUNTIME_SETTINGS.kioskMode) requestDisplayFullscreen();
  ...
  const diagnostics = createDiagnosticsState({ visible: diagnosticsVisible, displayId, presentationProfile: presentation.presentationProfile,
    startedAt: startedAt.current, now: clock, connectionState: presentation.connectionState, reconnectCount: presentation.reconnectCount,
    lastSuccessfulRefresh, hasError: Boolean(error), buildVersion: RUNTIME_SETTINGS.buildVersion });
  ...
  return <div className="app-shell">
    ...
    <SlideDeck data={data} displaySettings={displaySettings} slideIndex={presentation.activeSlideIndex} onSelectSlide={presentation.selectSlide} presentationState={{ hasError: Boolean(error), refreshing, rotationPaused }} />
