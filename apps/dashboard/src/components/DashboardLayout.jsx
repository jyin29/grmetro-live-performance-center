...
export function DashboardLayout({ data, displayId, error, refreshing, retry, lastSuccessfulRefresh }) {
  ...
  useEffect(() => {
    return attachWakeRecovery({ recover: () => watchdog.wake("browser-wake") });
  }, [watchdog]);
  useEffect(() => {
    let cursorTimer;
    ...
    showCursor();
    if (RUNTIME_SETTINGS.kioskMode) requestDisplayFullscreen();
