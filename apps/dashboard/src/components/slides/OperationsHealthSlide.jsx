...
function InstallEconomicsRows({ slide, metricVisibility }) {
  ...
  const visibleTechnicianCount = slide.rows.length;
  const density = visibleTechnicianCount <= 5 ? "short" : visibleTechnicianCount <= 7 ? "medium" : "dense";
  return <div className={`install-economics-list is-${density}-list`} aria-label="Install average ticket and revenue by technician" style={{ "--install-row-count": visibleTechnicianCount }}>{slide.rows.map((row) => <div className="install-economics-row" key={row.technicianId}><strong>{row.shortName || row.name}</strong><div className="install-economics-row__metrics">{metricIds.map((metricId) => { const definition = slide.metrics?.find((metric) => metric.id === metricId); const metric = metricFor(row, metricId); if (!definition) return null; return <div className="install-economics-row__metric" key={`${row.technicianId}-${metricId}`}><span>{definition.shortLabel || definition.label}</span><MetricValue metric={metric} /><i aria-hidden="true"><span style={{ width: `${visualRatio(slide, metricId, metric) * 100}%`, backgroundColor: definition.color }} /></i></div>; })}</div></div>)}</div>;
}
