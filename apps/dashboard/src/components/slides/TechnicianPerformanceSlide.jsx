...
export function TechnicianPerformanceSlide({ data, metricVisibility }) {
  ...
  return <main className="technician-performance dashboard-slide" aria-labelledby="technician-performance-title">
    <div className="technician-performance__heading">
      <div><p>{periodLabel} by person</p><h2 id="technician-performance-title">Technicians</h2></div>
      <span>Overall rank · Customized performance metrics</span>
    </div>
    <section className="technician-performance__grid" aria-label="Technician performance cards">
      {rankedTechnicians(data.technicians).map((technician) => <TechnicianPerformanceCard key={technician.id} technician={technician} data={data} metricVisibility={metricVisibility} />)}
    </section>
  </main>;
}
