...
export function MetricCustomization() {
  ...
  const save = async () => { try { const result = await saveDisplaySettings(state.settings); setState({ loading: false, settings: result.settings, saved: result.settings, status: "Metric visibility saved. Displays update within a few seconds.", error: false }); } catch (error) { setState((current) => ({ ...current, status: error.message, error: true })); } };

  return <div className="metric-customization">
    <div className="settings-title"><span className="mobile-eyebrow">Presentation layout · Phase 1</span><h2>Dashboard Metrics</h2><p>Choose which metrics appear on Slides 2, 3, and 4. Hidden metrics leave no empty space. Slides 1 and 5 come next.</p></div>
