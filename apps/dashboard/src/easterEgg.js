...
function isRefreshTarget(target) {
  if (!(target instanceof Element)) return false;
  return Boolean(target.closest(".refresh-dashboard-button"));
}
...
function showRefreshMessage() {
  if (refreshMessageActive) return;
  refreshMessageActive = true;
  const toast = document.createElement("div");
  toast.className = "refresh-easter-egg";
  toast.setAttribute("role", "status");
  toast.innerHTML = '<span>↻</span><div><strong>Why are you spamming me? 😭</strong><small>I promise I heard you the first time.</small></div>';
  document.body.appendChild(toast);
  requestAnimationFrame(() => toast.classList.add("is-visible"));
  window.setTimeout(() => toast.classList.remove("is-visible"), REFRESH_MESSAGE_MS - 500);
  window.setTimeout(() => { toast.remove(); refreshMessageActive = false; }, REFRESH_MESSAGE_MS);
}

document.addEventListener("pointerup", (event) => {
  const now = Date.now();
  if (isLogoTarget(event.target)) {
    logoTaps = logoTaps.filter((time) => now - time < LOGO_TAP_WINDOW_MS);
    logoTaps.push(now);
    if (logoTaps.length >= LOGO_TAP_COUNT) {
      logoTaps = [];
      celebrate();
    }
  }
  if (isRefreshTarget(event.target)) {
    refreshTaps = refreshTaps.filter((time) => now - time < REFRESH_TAP_WINDOW_MS);
    refreshTaps.push(now);
    if (refreshTaps.length >= REFRESH_TAP_COUNT) {
      refreshTaps = [];
      showRefreshMessage();
    }
  }
