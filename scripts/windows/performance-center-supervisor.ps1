...
$edgeLauncher = Join-Path $PSScriptRoot "launch-edge.ps1"
$script:backend = $null
...
function Get-AdminHealth {
  try { return Invoke-RestMethod -Uri $adminUrl -TimeoutSec 4 } catch { return $null }
}
function Start-Backend {
  ...
  $script:backend = Start-Process -FilePath "node" -ArgumentList $backendScript -WorkingDirectory $root -PassThru -WindowStyle Hidden -RedirectStandardOutput $stdoutLog -RedirectStandardError $stderrLog
...
function Wait-Healthy([int]$Seconds=30) {
  ...
  do { if(Test-BackendHealth){return $true}; if($script:backend -and $script:backend.HasExited){return $false}; Start-Sleep -Seconds 1 } while((Get-Date)-lt $deadline)
  return $false
}
...
function Restart-ServiceTitanBrowser([string]$Reason) {
  ...
  Write-SupervisorLog "browser-recovery" "Controlled ServiceTitan Edge recovery: $Reason" "WARN"
  try { & $edgeLauncher -RestartExisting; if($LASTEXITCODE -ne 0){throw "launch-edge exited $LASTEXITCODE"}; Start-Sleep -Seconds 8; Write-SupervisorLog "browser-relaunched" "ServiceTitan Edge launcher completed." "RECOVERED"; return $true }
...
function Invoke-FullRecovery([string]$Reason) {
  ...
  if(((Get-Date)-$script:lastFullRecovery).TotalMinutes -lt 10){ Write-SupervisorLog "full-recovery-loop-protected" "A full recovery already ran within 10 minutes. Intervention is required." "ERROR"; return $false }
...
Write-SupervisorLog "supervisor-start" "Windows supervisor started. Final architecture: central PC hosts backend/ServiceTitan; TVs independently load permanent display URLs."
...
try {
  while($true){
    ...
    if(-not (Test-BackendHealth)){
      $script:healthFailures += 1; $script:recoveryLevel=1; Write-SupervisorLog "health-miss" "Backend health check failed ($($script:healthFailures)/3 before restart)." "WARN"
      if($script:healthFailures -ge 3){ if(-not (Restart-BackendSafely "Three consecutive health failures")){ [void](Invoke-FullRecovery "Backend health remained unavailable") } }
      continue
    }
    if($script:healthFailures -gt 0){Write-SupervisorLog "health-recovered" "Backend recovered without process restart." "RECOVERED"}
    $script:healthFailures=0
    $admin=Get-AdminHealth
    $auth=Test-ServiceTitanAuthentication $admin
    if($auth -eq "login-required"){
      $script:recoveryLevel=6
      Write-SupervisorLog "intervention-required" "ServiceTitan authentication/login is required. Automatic destructive recovery is suspended to protect the session and logs." "ERROR"
      Start-Sleep -Seconds 30
      continue
    }
    if($auth -eq "degraded"){
      $script:serviceTitanFailures += 1
      Write-SupervisorLog "servicetitan-degraded" "ServiceTitan/browser diagnostics degraded ($($script:serviceTitanFailures)/6)." "WARN"
