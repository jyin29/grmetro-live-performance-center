...
The complete live investigation reported no validated native source for the four metrics, so Phase 10 uses the job-level derivation architecture. Service Revenue sums revenue from classified completed service jobs, Install Revenue sums revenue from classified completed installation jobs, Number of Installs counts those installation jobs, and Install Average Ticket divides that install revenue by the nonzero completed-install count. These results cannot be reconstructed safely from aggregate Revenue, Completed Jobs, Total Sales, Closed Opportunities, or name similarity alone.

The one-minute live refresh now runs the complete pipeline for each configured technician: Technician Overview → Technician Datasource → `TechnicianJobsExtendedDrilldownDatasource` → the shared job classifier → `deriveServiceInstallKpis()` → normalization and dashboard payload construction. The drilldown response is reduced through the existing allow-list sanitizer before classification, and the provider reuses the single classifier and derivation implementations in `apps/backend/src/data`.

`classificationApproved` remains `false`. The classifier and derivation execute on every successful live refresh, but the derivation boundary suppresses Billable Service Calls, Service Revenue, Install Revenue, Number of Installs, and Install Average Ticket to `hasData: false`, `value: null`, and `dataQuality: "unavailable"`. Development diagnostics, only when development routes are enabled, expose exactly `drilldownFetched`, `jobsProcessed`, `jobsClassified`, `serviceJobs`, `installJobs`, `excludedJobs`, `derivationExecuted`, and `outputSuppressed`; they contain no customer data, technician names, or revenue values.

The sole remaining Phase 10 step is business validation of the job-type rules, exclusions, completed-install definition, billable definition, and revenue basis. Once those rules have been populated and validated, changing only `classificationApproved` to `true` exposes the already-derived metrics as `derived`; no additional refresh-provider implementation is required.
...
The live provider attempts the configured five technicians with concurrency two. It validates Overview, Datasource, and job-drilldown responses, sanitizes drilldown rows to the approved job fields, runs the shared classifier and service/install derivation, and retains prior normalized technician data after an isolated failure. The derivation runs even while its output is suppressed by unapproved classification. Metadata requests are not part of the one-minute refresh. Lead Conversion mapping remains unimplemented pending Phase 11, which has not started.
