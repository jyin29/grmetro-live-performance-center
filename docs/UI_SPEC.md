...
The foundation is optimized for permanently mounted 65–85 inch displays with operations-center-scale titles, KPI values, labels, refresh status, chart text, leaderboard names, and footer text plus generous card padding and section spacing. Refresh age is continuously presented in seconds, or minutes and seconds, while retaining the established freshness colors. The existing dashboard is registered as the first entry in a reusable indexed slide deck that renders one full-screen slide at a time. A dedicated Technician Performance dashboard is registered as Slide 2. It presents one responsive card per technician with backend-prepared overall rank, rank movement, Revenue, Closing %, Billable Calls, Install Revenue, Average Ticket, and the source KPI data-quality states. The cards preserve unavailable install values as `No data` and do not calculate new business meaning.

The fifth and final Phase 11 presentation slide is Operations Health. It gives dashboard health one dominant status statement, followed by large cards for last successful refresh and age, refresh state, cache availability, configured payload technician count, current slide, and local presentation rotation. These values are presentation-only interpretations of state already held by the dashboard; no additional endpoint or request is used. Missing state is shown as `Unavailable` rather than inferred.

The indexed deck automatically rotates through every currently registered dashboard slide on a presentation-only 30-second interval. It begins on Slide 1 and wraps after the final registered slide. Loading, background refresh, and error states pause the timer; rotation resumes automatically after those states clear. A 400 ms opacity-only crossfade keeps the shell stationary, and a subdued keyboard-accessible position indicator identifies the active slide. Shared-epoch synchronization, display routing, WebSocket state, and remote overrides remain deferred to their dedicated tasks. Existing 250–450 ms ease-out entrance treatments and the reduced-motion override remain intact.

Slide 3 is a dedicated Business Performance presentation view. It places a large Revenue by Technician overlay visualization beside a large Closing Performance comparison, reusing the Revenue and Performance slide payloads without calculating team totals, contribution percentages, leaders, or other business meaning in the frontend. Its low-density, large-type layout is optimized for approximately three-second comprehension from ten feet and scales down to the approved 720p minimum.

Slide 4 is a dedicated Recognition & Achievements presentation view. One backend-qualified overall rank-one technician occupies most of the slide beneath the title `Today’s Top Performer`, with large Revenue, Closing %, and Overall Rank values. A secondary row may show Highest Revenue, Biggest Rank Improvement, Highest Closing %, and Top Overall Ranking only when each recognition is directly supported by backend-prepared KPI ranks, overall rank, or rank movement. The slide adds no KPI calculations or business scoring and reuses the shared formatting, value animation, deck transition, keyboard navigation, responsive, and reduced-motion systems.

Fresh REST payloads animate changed numeric KPI, revenue, technician, and ranking values with a 650 ms non-overshooting ease-out interpolation. Existing SVG revenue and comparison bars transition their dimensions in place rather than rebuilding chart structures. Changed KPI and technician cards receive a restrained teal tint, while leaderboard rows receive the same subtle emphasis when rank data changes; each treatment clears after approximately one second. Reduced-motion preferences make all value and chart updates immediate. A successful payload's refresh timestamp immediately presents `Updated 0 sec ago`, then advances once per second.
