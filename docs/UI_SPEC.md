...
The foundation is optimized for permanently mounted 65–85 inch displays with selectively enlarged titles, values, labels, status text, chart text, and leaderboard names plus increased card padding and section spacing. The existing dashboard is registered as the first entry in a reusable indexed slide deck that renders one full-screen slide at a time. This foundation intentionally has no timer, keyboard navigation, WebSocket state, or automatic rotation. Entrance motion uses restrained 200–500 ms opacity and small vertical-offset transitions for cards, KPI values, charts, and leaderboard rows, with the existing reduced-motion override retained.
