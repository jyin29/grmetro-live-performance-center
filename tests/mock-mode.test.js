...
const enabledConfig = loadMockConfig({ NODE_ENV: "development", MOCK_MODE: "true", ENABLE_DEVELOPMENT_ROUTES: "true" });
const DATA_GROUP_IDS = ["revenue", "activity", "performance", "average-ticket", "top-three"];
...
test("every scenario produces configured technicians, KPI records, and dashboard data groups", async () => {
  for (const scenario of MOCK_SCENARIO_NAMES) {
    ...
    assert.deepEqual(Object.keys(payload.slides), DATA_GROUP_IDS, scenario);
...
test("partial failure and stale scenarios retain five technician records with explicit diagnostics", async () => {
